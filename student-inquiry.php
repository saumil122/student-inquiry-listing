...
$student_inquiry_db_version = '1.0';

/**
 * register_activation_hook implementation
 */
function student_inquiry_install()
{
    global $wpdb;
    global $student_inquiry_db_version;
...
register_activation_hook(__FILE__, 'student_inquiry_install');

/**
 * register_activation_hook implementation
 */
function student_inquiry_install_data()
{
    global $wpdb;
...
register_activation_hook(__FILE__, 'student_inquiry_install_data');

/**
 * Trick to update plugin database
 */
function student_inquiry_update_db_check()
{
    global $student_inquiry_db_version;
    if (get_site_option('student_inquiry_db_version') != $student_inquiry_db_version) {
        student_inquiry_install();
    }
}
add_action('plugins_loaded', 'student_inquiry_update_db_check');
...
class Student_Inquiry_List_Table extends WP_List_Table
{
    ...
    {
        $actions = array(
            'edit' => sprintf('<a href="?page=inquiries_form&id=%s">%s</a>', $item['id'], __('Edit', 'student_inquiry')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['id'], __('Delete', 'student_inquiry')),
        );
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'name' => __('Name', 'student_inquiry'),
            'email' => __('Email', 'student_inquiry'),
            'subject' => __('Subject', 'student_inquiry'),
            'message' => __('Message', 'student_inquiry'),
            'course' => __('Course', 'student_inquiry'),
            'amount' => __('Amount', 'student_inquiry'),
            'status' => __('Status', 'student_inquiry'),
        );
        return $columns;
    }
    ...
    {
        $actions = array(
            'delete' => __('Delete', 'student_inquiry')
        );
        return $actions;
    }
 ...
 */
function student_inquiry_admin_menu()
{
    add_menu_page(__('Inquiries', 'student_inquiry'), __('Inquiries', 'student_inquiry'), 'activate_plugins', 'inquiries', 'student_inquiries_page_handler');
    add_submenu_page('inquiries', __('Inquiries', 'student_inquiry'), __('Inquiries', 'student_inquiry'), 'activate_plugins', 'inquiries', 'student_inquiries_page_handler');
    // add new will be described in next part
    add_submenu_page('inquiries', __('Add new', 'student_inquiry'), __('Add new', 'student_inquiry'), 'activate_plugins', 'inquiries_form', 'student_inquiries_form_page_handler');
    add_submenu_page('inquiries', __('Send email', 'student_inquiry'), __('Send email', 'student_inquiry'), 'activate_plugins', 'inquiries_send_email', 'student_inquiries_send_email_page_handler');
}

add_action('admin_menu', 'student_inquiry_admin_menu');

/**
 * List page handler: Render the table
 * Look into /wp-admin/includes/class-wp-*-list-table.php for examples
 */
function student_inquiries_page_handler()
{
    global $wpdb;
 ...
 */
function student_inquiries_form_page_handler()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'inquiries';
    ...
    if (isset($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
        ...
        $item_valid = student_inquiry_validate_inquiry($item);
        if ($item_valid === true) {
            if ($item['id'] == 0) {
                $result = $wpdb->insert($table_name, $item);
                $item['id'] = $wpdb->insert_id;
                if ($result) {
                    $message = __('Item added successfully.', 'student_inquiry');
                } else {
                    $notice = __('There was an error while adding an item.', 'student_inquiry');
                }
            } else {
                $result = $wpdb->update($table_name, $item, array('id' => $item['id']));
                if ($result) {
                    $message = __('Item updated successfully.', 'student_inquiry');
                } else {
                    $notice = __('There was an error while updating an item.', 'student_inquiry');
                }
            }
        } else {
        ...
        if (isset($_REQUEST['id'])) {
            ...
            if (!$item) {
                $item = $default;
                $notice = __('Item is not found.', 'student_inquiry');
            }
        }
    }

    // here we adding our custom meta box
    add_meta_box('inquiries_form_meta_box', 'Inquiry data', 'student_inquiries_form_meta_box_handler', 'inquiry', 'normal', 'default');
 ...
 */
function student_inquiries_form_meta_box_handler($item)
{
?>
 ...
 * validates data 
 */
function student_inquiry_validate_inquiry($item)
{
    $messages = array();

    if (empty($item['name'])) $messages[] = __('Please provide valid name.', 'student_inquiry');
    if (!empty($item['email']) && !is_email($item['email'])) $messages[] = __('Please provide valid email.', 'student_inquiry');
    if (empty($item['subject'])) $messages[] = __('Please provide valid subject.', 'student_inquiry');
    if (empty($item['message'])) $messages[] = __('Please provide valid message.', 'student_inquiry');
 ...
 ***/
function sn_send($email, $template = null, $from = '', $subject = 'Inquiry regarding class', $args = array())
{
    ...
    $message = sn_convert_tags($message, $args);
 ...
 ***/
function sn_convert_tags($content, $args = array())
{
 ...
 */
function student_inquiries_send_email_page_handler()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'inquiries';
        ...
        if (isset($item['selected_ids']) && $item['selected_ids'] != '') {
            ...
            if (isset($records) && !empty($records) && count($records) > 0) {
                ...
                foreach ($records as $record) {
                    ...
                    sn_send( $record['email'], '', $item['from'], $item['subject'], $args);
                }

                $message = __('Invite send successfully.', 'student_inquiry');
            } else {
                $notice = __('Oops! something went wrong. Please try again.', 'student_inquiry');
            }
        } else {
            $notice = __('Please select student to send invite.', 'student_inquiry');
        }
    } else {
        $item = $default;
    }
    ...
    // here we adding our custom meta box
    add_meta_box('inquiries_send_meta_box', 'Send invite email', 'student_inquiries_send_meta_box_handler', 'inquiry', 'normal', 'default');
 ...
 * renders the custom meta box
 */
function student_inquiries_send_meta_box_handler($item)
{
?>
    <table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
        <tbody>
...
add_action('admin_enqueue_scripts', function () {
    global $student_inquiry_db_version;
    wp_enqueue_style('jquery-auto-complete', plugin_dir_url(__FILE__) . 'css/jquery.auto-complete.css', array(),  $student_inquiry_db_version);
    wp_enqueue_script('autocomplete-js', plugin_dir_url(__FILE__) . 'js/jquery.auto-complete.min.js', array('suggest'), $student_inquiry_db_version, true);
 ...
 */
function student_inquiry_languages()
{
    load_plugin_textdomain('student_inquiry', false, dirname(plugin_basename(__FILE__)));
}

add_action('init', 'student_inquiry_languages');


/* shortcode for inquiry form */
function generate_inquiry_form()
{
    if ($_GET['action'] != 'edit') { ?>
            ...
            <form id="inquiry_form" class="inquiry_form" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST" enctype="multipart/form-data">
                <fieldset>
                    <div class="field_wrapper">
                        <label for="inquiry_name"><?php echo __('Name', 'sn') ?></label>
                        <div class="field">
                            <input name="inquiry_name" id="inquiry_name" type="text" />
                        </div>
                    </div>
                    <div class="field_wrapper">
                        <label for="inquiry_email"><?php echo __('Email address', 'sn') ?></label>
                        <div class="field">
                            <input name="inquiry_email" id="inquiry_email" type="email" />
                        </div>
                    </div>
                    <div class="field_wrapper">
                        <label for="inquiry_subject"><?php echo __('Subject', 'sn') ?></label>
                        <div class="field">
                    ...
                    <div class="field_wrapper">
                        <label for="inquiry_message"><?php echo __('Message', 'sn') ?></label>
                        <div class="field">
                    ...
                    <div class="field_wrapper">
                        ...
                        <div class="field">
                            ...
                            <input type="submit" id="inquiry_form_btn" value="<?php echo __('Submit', 'sn') ?>" />
                        </div>
                    </div>
                </fieldset>
            </form>
...
add_shortcode('INQUIRY_FORM', 'generate_inquiry_form');

function save_student_inquiry_data()
{
    $result = array(
        'error' => false
    );
...
add_action('wp_ajax_inquiryform_submit', 'save_student_inquiry_data');
add_action('wp_ajax_nopriv_inquiryform_submit', 'save_student_inquiry_data');
